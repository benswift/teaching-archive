---
title: "Week 1: hello sine"
summary: your first beeps and boops
---

There isn't much pre-reading for week 1, since the class hasn't really started
yet. The main purpose of the first workshop is to get [Pd](/resources/tools/#pure-data) up and running on everyone's laptop---because
if you don't have that working then you won't be able to participate in this course.

This first workshop is called "hello sine" because making
[pure sine wave](https://en.wikipedia.org/wiki/Pure_tone) is pretty much the
["Hello, World!"](https://en.wikipedia.org/wiki/%22Hello,_World!%22_program) of
computer music.

## Goals for this week

1. to be able to make a noise with Pd
2. to be able to make a screen recording (with sound!)

## Pd resources

- [installing Pd on your laptop](/resources/tools/#pure-data)

- getting started with Pd (Charles) ([YouTube](https://youtu.be/AC7-MpwIZ8w))
  (don't worry if you don't follow all of the stuff in this video---it includes
  a lot of the things we'll cover [next week](/workshops/synthesis-1/) as well)

If you get stuck, try asking for help in the `#techsupport` channel on Teams.

## Things to think about

Just for inspo, have a look at some of the most famous Laptop Orchestras (the
cool kids call them <em>LOrk</em>s) perform live.

- [Stanford Laptop Orchestra - Twilight (2013)](https://www.youtube.com/watch?v=chA-4GRCb-I)
- [Princeton Laptop Orchestra - Interference (2019)](https://www.youtube.com/watch?v=C-5P3hXuGfs)

If you find any more cool videos, drop them in the `#inspo` channel on Teams.
