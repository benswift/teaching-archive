...
_Note: the bassline starts at around 0:23_
...
_Note: this tune is based on the sound signature of the lyrics (throughout), and also the
note progression which starts at around 6:30_
...
For questions around frequency accuracy, _please read_ [this section](#how-close)
...
that it's ok if you don't do something _super_-complex, what really matters is
...
For more tips on _how_ to write a design document,
...
4. the last commits on the `part-1` and `part-2` branches [on the GitLab
   ...
   (not on your local machine!) _before the submission deadline_ will count as
   your submission
...
3. my `statement-of-originality.yml` files for both Part 1 and Part 2 include
   [all the necessary references/acknowledgements](/resources/01-faq/#statement-of-originality), and _everything_ not mentioned in there is my own work
...
As close as you can get! You can probably make it _fairly_ close without too
...
_Note: The sequence you generate in part 2 doesn't have to be a song---there are
lots of ways to make interesting patterns in frequency & time_

A **note** has three basic dimensions: the _pitch_, the _loudness_ and the
_duration_. If you want to play a part of your favourite song, you need to find
...
- by playing successive notes with slightly different loudness, you can
  create [_accents_](<https://en.wikipedia.org/wiki/Accent_(music)>)
...
- the relationship between frequency and the notes on the piano keyboard isn't
  ...
  _is_ linear, and you can use this handy conversion widget (or write an asm
...
Your program can use _any_ part of the discoboard---as long as it still meets the
