...
storing data. There are hundreds of registers on the discoboard, but you'll use
some of them more than others. Because the discoboard is a 32-bit machine most
...
registers for configuring and controlling the peripherals on your discoboard.
For example, you configure the clocks and turn on the LEDs on your discoboard by
writing to special GPIO registers. If you're after a refresher course on this
stuff, [lab 5](/labs/05-blinky/) has lots more
information.
...
space" of a computer (e.g. your discoboard) is a mapping from addresses (which
are just numbers) to different memory addresses on the discoboard. On your
32-bit discoboard (and on any 32-bit CPU) there are $$2^{32}$$ possible memory
...
The discoboard's [memory space](#memory-address-space) is addressed in bytes, so
...
Confusingly, though, on your **little-endian** discoboard the least significant
...
the discoboard, the interrupt controller is called the Nested Vectored Interrupt
