...
An edge is a line segment (or a Vector, *or sometimes a curve*) connecting 2 vertices. 
...
A [***normal***](https://en.wikipedia.org/wiki/Normal_(geometry)) is a vector that indicates a direction which is perpendicular [*at right angle to*] a given object.
...
:::info
**NOTE:** inside the `beginGeometry()` and `endGeometry()` you issue commands to ***draw*** a 3D shape. 
...
Once you have created your geometry, you can render it *(draw it)* using the command:
...
Can you add in an externally created 3D model *[such as Suzanne]*, and combine this with built-in shapes?<br>
...
function arrow3D() { // start building our custom geometry
...
For smooth *curvy* shading, each vertex must have a vertex normal defined.
