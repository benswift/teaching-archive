...
- The shader is not _pushed_ to the geometry (often we use a `rect()` to push the shader)
...
| :------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------- |
| Automatic handling of vertex shader<br>This also means that `attribute` and `varying` variables are not needed. This is the same as for a _Processing_ shader. | specific vertex shader file required                                                         |
| no precision definition                                                                                                                                        | requires a specification of floating point precision, <br>such as `precision mediump float;` |
